...
export default function DishPhotoUpload({ existingPhotos, userId, onPhotosChange }: DishPhotoUploadProps) {
  ...
  // Check if chef-dishes bucket exists
  useEffect(() => {
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (photos.length + files.length > MAX_PHOTOS) {
      setError(`You can upload a maximum of ${MAX_PHOTOS} dish photos`);
      return;
    }
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      uploadPromises.push(
        uploadDishPhoto(file, userId, tempId, newPreviewUrls, newPhotos)
      );
    }
  ...
  const uploadDishPhoto = async (
    ...
    try {
      ...
      console.log(`Uploading dish photo: ${file.name}, type: ${file.type}, size: ${(file.size / 1024).toFixed(2)} KB`);
      ...
      console.log("Dish photo uploaded successfully");
    } catch (err) {
  ...
  const handleDeletePhoto = async (photoUrl: string) => {
    try {
      ...
      toast({
        title: "Photo deleted",
        description: "The dish photo was successfully removed."
      });
    } catch (err) {
    ...
    <div className="space-y-4 w-full">
      <div className="flex flex-col space-y-2">
        <div className="flex justify-between items-center">
          <h3 className="text-lg font-medium">Dish Photos</h3>
        ...
        <p className="text-sm text-neutral-500">
          Showcase your culinary creations to potential clients
        </p>
      ...
      {Object.keys(previewUrls).length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mt-4">
          ...
          {Object.keys(previewUrls).length === 0 && (
            <div className="col-span-full flex flex-col items-center justify-center p-8 border border-dashed rounded-md bg-neutral-50">
              <Image className="h-12 w-12 text-neutral-300 mb-2" />
              <p className="text-neutral-500 text-center">
                No dish photos uploaded yet
              </p>
            </div>
          )}
        </div>
